...
def login(mosq,obj,msg):
    ...
    try:
       ...
       sqlq = "SELECT ImgId,Version,ClientId FROM AC_Users WHERE Username='"+username+"' AND Password='"+password+"'"
       ...
       if results > 0:
          ...
          output = '{"function":"validate_login","session_id":"%s","error_code":"0", "Response":"Successfully Authenticated for user: %s","img_path":"%s","version":"%s","cname":"%s"}' %(sid,username,imgpath,version,cname)
          #print output
    ...
    try:
       ...
       if((data1.get('history') is None) or ((data1.get('history') is not  None) and (len(data1['history']) <= 0))):
           ...
           if history == "all_cities":
              ...
              # it returns when input unit exists
              sqlq2="SELECT  i.UnitId,i.IpMacid,i.IpName,o.OpMacid,o.OpName,p.PMacId,p.PName,o.Fan,o.Status,i.Temp,o.setTemp,o.Voltage,o.Power,o.Current,p.Voltage,p.Power,p.Current,o.UnitId,p.UnitId FROM  AC_Input_Units i LEFT JOIN AC_Operate_Units o  USING (UnitId,PremiseId) LEFT JOIN AC_Power_Units p  USING (UnitId,PremiseId) where PremiseId='%s' ORDER BY i.UnitId,o.UnitId,p.UnitId" %(pid) 
...
# UnitId | IpMacid      | IpName    | OpMacid      | OpName    | PMacId       | PName      | Fan  | Status | Temp | setTemp | Voltage | Power | Current | Voltage | Power | Current | UnitId | UnitId |
       ...
       if history_all == True:
          ...
          sqlq3 = "select round(Avg(Temp))as temp from AC_Input_Units where ChangeBy='%s' group by CityId" %(username)
          ...
          sqlq4="select CityId,CityDesc,setTemp from AC_City where ChangeBy='%s' " %(username)
             ...
             for i,rec in enumerate(get_units_rec):
     		 ...
     		 if i < len(main):
                 ...
                 if(counter == 1):
                    ...
                    output += '{"city_id":"%s","CityDesc":"%s","setTemp":"%s","avgtemp":"%s","good":"%s","warning":"%s","problem":"%s","maintenance":"%s"}' %(rec[0] ,rec[1],rec[2],citytemp,goodval,warningval,problemval,mainval)
                 else:
                    
                    output += ',\n {"city_id":"%s","CityDesc":"%s","setTemp":"%s","avgtemp":"%s","good":"%s","warning":"%s","problem":"%s","maintenance":"%s"}' %(rec[0] ,rec[1],rec[2],citytemp,goodval,warningval,problemval,mainval)
                 #print output
             output += ']\n'
             output += '}'
             #print output
       ...
       if history_city == True:
          ...
          if((data1.get('city_id') is None) or ((data1.get('city_id') is not  None) and (len(data1['city_id']) <= 0))):
             ...
             main = cursor.fetchall()
             #print "response fro cld maintenance : ",main,len(main)
             sqlq3 = "select round(Avg(Temp))as temp from AC_Input_Units where CityId='%s' and ChangeBy='%s' group by PremiseId" %(cid,username)  
             ...
             sqlq2="select PremiseId,PremiseDesc,setTemp from AC_Premise where CityId='%s' and ChangeBy='%s'" %(cid,username)
             ...
             if(len(get_units_rec) > 0):
   		    ...
   		    #print warningval
                    ...
                    counter += 1
                    if(counter == 1):
                       output += '{"city_id":"%s","premise_id":"%s","premise_desc":"%s","setTemp":"%s","avgtemp":"%s","good":"%s","warning":"%s","problem":"%s","maintenance":"%s"}' %(cid,rec[0] ,rec[1],rec[2],pretemp,goodval,warningval,problemval,mainval)
                    else:
                       output += ',\n {"city_id":"%s","premise_id":"%s","premise_desc":"%s","setTemp":"%s","avgtemp":"%s","good":"%s","warning":"%s","problem":"%s","maintenance":"%s"}' %(cid,rec[0] ,rec[1],rec[2],pretemp,goodval,warningval,problemval,mainval)
                output += ']\n'
                output += '}'
                #print output
                return mqttc.publish('jts/oyo/error',output)
...
# UnitId | IpMacid      | IpName    | OpMacid      | OpName    | PMacId       | PName      | Fan  | Status | Temp | setTemp | Voltage | Power | Current | Voltage | Power | Current | UnitId | UnitId |
       ...
       if history_all == True:
          ...
          sqlq="SELECT * FROM (SELECT IFNULL( (select count(*)as count  from AC_Input_Units where Temp between 23 and 25 and ChangeBy='%s' GROUP BY CityId) ,'0') as good) A, (SELECT IFNULL( (select count(*)as count  from AC_Input_Units where Temp between 25 and 27 and ChangeBy='%s' GROUP BY CityId) ,'0') as warning) B, (SELECT IFNULL( (select count(*)as count  from AC_Input_Units where Temp between 20 and 22 and ChangeBy='%s' GROUP BY CityId) ,'0') as problem) C, (SELECT IFNULL( (select count(*)as count from AC_Input_Units where Temp=50 and ChangeBy='%s' GROUP BY CityId) ,'0') as main) D,(SELECT IFNULL( (select round(Avg(Temp))as temp from AC_Input_Units where ChangeBy='%s' group by CityId) ,'0') as avgTemp) E" %(username,username,username,username,username)
          cursor.execute(sqlq)
             ...
             for rec in get_units_rec:
     		 ...
     		 if i < len(main):
                 ...
                 if(counter == 1):
                    ...
                    output += '{"city_id":"%s","CityDesc":"%s","avgtemp":"%s","good":"%s","warning":"%s","problem":"%s","maintenance":"%s"}' %(rec[0] ,rec[1],datamain[0][4],datamain[0][0],datamain[0][1],datamain[0][2],datamain[0][3])
                 else:
                    
                    output += ',\n {"city_id":"%s","CityDesc":"%s","avgtemp":"%s","good":"%s","warning":"%s","problem":"%s","maintenance":"%s"}' %(rec[0] ,rec[1],datamain[0][4],datamain[0][0],datamain[0][1],datamain[0][2],datamain[0][3])
                 #print output
             output += ']\n'
             output += '}'
             #print output
...
def delete_unit(mosq,obj,msg):
       ...
       try:
          ...
          return mqttc.publish('jts/oyo/error',str(res))
           ...
           output = '{"function":"delete_unit","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete unit"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)
...
def delete_unit_utype(mosq,obj,msg):
       ...
       try:
          ...
          return mqttc.publish('jts/oyo/error',str(res))
           ...
           output = '{"function":"delete_unit_utype","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete unit type"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)
###################### delete room ##########################################
def delete_room(mosq,obj,msg):
       ...
       try:
          ...
          return mqttc.publish('jts/oyo/error',str(res))
           ...
           output = '{"function":"delete_room","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete room"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)
###################### delete floor ##########################################
def delete_floor(mosq,obj,msg):
       ...
       try:
          ...
          return mqttc.publish('jts/oyo/error',str(res))
           ...
           output = '{"function":"delete_floor","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete floor"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)
...
def delete_city(mosq,obj,msg):
       ...
       try:
          ...
          return mqttc.publish('jts/oyo/error',str(res))
           ...
           output = '{"function":"delete_city","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete city"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)
################# delete premise ##############################################
def delete_premise(mosq,obj,msg):
       #print "delete premise......."
       ...
       try:
          ...
          return mqttc.publish('jts/oyo/error',str(res))
           ...
           output = '{"function":"delete_premise","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete premise"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)
...
#################### get Graph history for mobile# ##########################################
def get_graph_history_mobile(mosq,obj,msg):
    ...
    try:
       ...
       if graph_Month==True:
          ...
          sqlq1="select avg(i.Temp) as Temp,o.MotionE,avg(p.Power) as Power,i.ChangeDate from AC_Input_History as i use index(UnitId_idx,ChangeDate_idx) left join AC_Operate_History as o use index(UnitId_idx,ChangeDate_idx) USING (UnitId,ChangeDate) left join AC_Power_History as p use index(UnitId_idx,ChangeDate_idx) USING (UnitId,ChangeDate)  where UnitId='%s' and (i.ChangeDate>='%s' or o.ChangeDate>='%s' or p.ChangeDate>='%s') group by day(ChangeDate) order by ChangeDate" %(uid,month_old_date,month_old_date,month_old_date)
          cursor.execute(sqlq1)
          ...
          if len(get_temp) > 0:
             ...
             for rec in get_temp:
                 ...
                 #print stdate
                 stdate= datetime.strftime(rec[3], '%Y-%m-%d %H:%M')
                 #print stdate
                 if(counter == 1):
                    output += '{"temp":"%s","power":"%s","eco":"%s","changedate":"%s"}' %(rec[0], rec[2],rec[1],stdate)
                 else:
                    output += ',\n {"temp":"%s","power":"%s","eco":"%s","changedate":"%s"}' %(rec[0] ,rec[2],rec[1],stdate)
             output += ']\n'
       ...
       if graph_Day==True:
          ...
          #sql="select i.Temp,o.MotionE,p.Power,i.ChangeDate from AC_Input_History as i left join AC_Operate_History as o USING (ChangeDate) left join AC_Power_History as p USING (ChangeDate)  where i.IpId='%s' and o.OpId='%s' and p.PId='%s' and (i.ChangeDate>curdate() or o.ChangeDate>curdate() or p.ChangeDate>curdate()) group by hour(ChangeDate),minute(ChangeDate)" %(iid,oid,pid)
          #print sql
          sql="select avg(i.Temp) as Temp,o.MotionE,avg(p.Power) as Power,FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(i.ChangeDate)/(10*60))*(10*60)) as timeint from AC_Input_History as i use index(UnitId_idx,ChangeDate_idx) left join AC_Operate_History as o use index(UnitId_idx,ChangeDate_idx) USING (UnitId,ChangeDate) left join AC_Power_History as p use index(UnitId_idx,ChangeDate_idx) USING (UnitId,ChangeDate)  where UnitId='%s' and (i.ChangeDate>curdate() or o.ChangeDate>curdate() or p.ChangeDate>curdate()) group by timeint order by ChangeDate" %(uid)
	  cursor.execute(sql)
          get_temp = cursor.fetchall()
          #if len(get_temp) > 0  or len(get_power) > 0 or len(get_voltage) > 0:
          #   output = '{"function":"get_graph_history","session_id":"%s","error_code":"0", "Response":"Success",' %(sid)
       ...
       if graph_Year==True:
          ...
          #print(year_old_date)
          sqlq1="select avg(i.Temp) as Temp,o.MotionE,avg(p.Power) as Power,i.ChangeDate from AC_Input_History as i use index(UnitId_idx,ChangeDate_idx) left join AC_Operate_History as o use index(UnitId_idx,ChangeDate_idx) USING (UnitId,ChangeDate) left join AC_Power_History as p use index(UnitId_idx,ChangeDate_idx) USING (UnitId,ChangeDate)  where UnitId='%s' and (i.ChangeDate>='%s' or o.ChangeDate>='%s' or p.ChangeDate>='%s') group by month(ChangeDate) order by ChangeDate" %(uid,year_old_date,year_old_date,year_old_date)
          cursor.execute(sqlq1)
          get_temp = cursor.fetchall()
          #if len(get_temp) > 0  or len(get_power) > 0 or len(get_voltage) > 0:
          #   output = '{"function":"get_graph_history","session_id":"%s","error_code":"0", "Response":"Success",' %(sid)
...
def get_graph_history(mosq,obj,msg):
    ...
    try:
       ...
       if graph_Month==True:
          ...
          #print(month_old_date)
          if ipid==True:
             sqlq1="select Avg(Temp) as Temp,ChangeDate from AC_Input_History use index(ChangeDate_idx) WHERE IpId='%s' and ChangeDate >= '%s' GROUP BY DAY(ChangeDate) ORDER BY IHId" %(iid,month_old_date)
             #print sqlq1
             cursor.execute(sqlq1)
             get_temp = cursor.fetchall()
             #print "temp .....",get_temp
             ...
             get_temp=''
          if opid==True:
             sqlq2="select Avg(Power) as power,ChangeDate from AC_Operate_History use index(ChangeDate_idx) WHERE OpId='%s' and ChangeDate >= '%s' GROUP BY DAY(ChangeDate) ORDER BY OHId" %(oid,month_old_date)
             cursor.execute(sqlq2)
             get_power = cursor.fetchall()
             #print "operate Power.......",get_power
             sqlq3="select Avg(MotionE) as motion,ChangeDate from AC_Operate_History use index(ChangeDate_idx) WHERE OpId='%s' and ChangeDate >= '%s' GROUP BY DAY(ChangeDate) ORDER BY OHId" %(oid,month_old_date)
             cursor.execute(sqlq3)
             ...
             get_voltage=''
          if ppid==True:
             sqlq2="select Avg(Power) as power,ChangeDate from AC_Power_History use index(ChangeDate_idx) WHERE PId='%s' and ChangeDate >= '%s' GROUP BY DAY(ChangeDate) ORDER BY PHId" %(pid,month_old_date)
             cursor.execute(sqlq2)
             get_power = cursor.fetchall()
       ...
       if graph_Day==True:
          ...
          if ipid==True:
             ...
             sqlq1="select Avg(Temp) as Temp,FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ChangeDate)/(10*60))*(10*60)) AS time_int from AC_Input_History use index(ChangeDate_idx) WHERE IpId='%s' and ChangeDate > CURDATE() GROUP BY time_int ORDER BY IHId" %(iid)
             ...
             get_temp=''
          if opid==True:
             sqlq2="select Avg(Power) as power,FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ChangeDate)/(10*60))*(10*60)) AS time_int from AC_Operate_History use index(ChangeDate_idx) WHERE OpId='%s' and ChangeDate > CURDATE() GROUP BY time_int ORDER BY OHId" %(oid)
             cursor.execute(sqlq2)
             get_power = cursor.fetchall()
             #print "operate Power.......",get_power
             sqlq3="select Avg(MotionE) as motion,FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ChangeDate)/(10*60))*(10*60)) AS time_int from AC_Operate_History use index(ChangeDate_idx) WHERE OpId='%s' and ChangeDate > CURDATE() GROUP BY time_int ORDER BY OHId" %(oid)
             cursor.execute(sqlq3)
             get_voltage = cursor.fetchall()
             #print "operate voltage .......",get_voltage
          else:
             ...
             get_voltage=''
          if ppid==True:
             sqlq2="select Avg(Power),FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP(ChangeDate)/(10*60))*(10*60)) AS time_int from AC_Power_History use index(ChangeDate_idx) WHERE PId='%s' and ChangeDate > CURDATE() GROUP BY time_int ORDER BY PHId" %(pid)
             cursor.execute(sqlq2)
             get_power = cursor.fetchall()
             #print "Power.......",get_power
       ...
       if graph_Year==True:
          ...
          #print(year_old_date)
          if ipid==True:
             sqlq1="select Avg(Temp) as Temp,ChangeDate from AC_Input_History use index(ChangeDate_idx) WHERE IpId='%s' and ChangeDate >= '%s' GROUP BY MONTH(ChangeDate) ORDER BY IHId" %(iid,year_old_date)
             #print sqlq1
             cursor.execute(sqlq1)
             get_temp = cursor.fetchall()
             #print "temp .....",get_temp
             ...
             get_temp=''
          if opid==True:
             sqlq2="select Avg(Power) as power,ChangeDate from AC_Operate_History use index(ChangeDate_idx) WHERE OpId='%s' and ChangeDate >= '%s' GROUP BY MONTH(ChangeDate) ORDER BY OHId" %(oid,year_old_date)
             cursor.execute(sqlq2)
             get_power = cursor.fetchall()
             #print "operate Power.......",get_power
             sqlq3="select Avg(MotionE) as motion,ChangeDate from AC_Operate_History use index(ChangeDate_idx) WHERE OpId='%s' and ChangeDate >= '%s' GROUP BY MONTH(ChangeDate) ORDER BY OHId" %(oid,year_old_date)
             cursor.execute(sqlq3)
             get_voltage = cursor.fetchall()
             #print "operate voltage .......",get_voltage
          else:
             ...
             get_voltage=''
          if ppid==True:
             sqlq2="select Avg(Power),ChangeDate from AC_Power_History use index(ChangeDate_idx) WHERE PId='%s' and ChangeDate >= '%s'  GROUP BY MONTH(ChangeDate) ORDER BY PHId" %(pid,year_old_date)
             cursor.execute(sqlq2)
             get_power = cursor.fetchall()
             #print "Power.......",get_power
...
def get_saved_units(mosq,obj,msg):
    ...
    try:
       ...
       if graph_Month==True:
          sqlq2 = "select sum(Power)/1000 as total_units , date(ChangeDate) as Date from AC_Power_History where PId='%s' and ChangeDate<curdate() group by day(ChangeDate) order by ChangeDate" %(pid)
          cursor.execute(sqlq2)
          get_power_rec = cursor.fetchall()
          #print get_power_rec[1]
          sqlq3 = " select Saved,date(ChangeDate) as Date from AC_Saved_Units where UnitId='%s' and ChangeDate<curdate() order by ChangeDate" %(uid)
          cursor.execute(sqlq3)
       ...
       if graph_Year==True:
          sqlq2 = "select sum(Power)/1000 as total_units , date(ChangeDate) as Date from AC_Power_History where PId='%s' and ChangeDate<curdate() group by month(ChangeDate) order by ChangeDate" %(pid)
          cursor.execute(sqlq2)
          get_power_rec = cursor.fetchall()
          #print get_power_rec[1]
          sqlq3 = " select Saved,date(ChangeDate) as Date from AC_Saved_Units where UnitId='%s' and ChangeDate<curdate() group by month(ChangeDate) order by ChangeDate" %(uid)
          cursor.execute(sqlq3)
          get_saved_units_rec = cursor.fetchall()
          #print get_saved_units_rec[1]
          if(len(get_power_rec) > 0):
