...
def get_power(mosq,obj,msg):
       ...
       try:
          ...
          sqlq1 = "select avg(power) from Oyo_Unit_History where MotionE=1"
          cursor.execute(sqlq1)
          results1 = cursor.fetchone()
          sqlq2 = "select avg(power) from Oyo_Unit_History where MotionE=0"
          cursor.execute(sqlq2)
          results2 = cursor.fetchone()
...
def get_temp_history(mosq,obj,msg):
    ...
    try:
       ...
       sqlq2="SELECT UnitId,format(avg(Temp),0) as temp ,ChangeDate FROM Oyo_Unit_History WHERE ChangeDate>='%s' and ChangeDate <= '%s' GROUP BY HOUR(ChangeDate),MINUTE(ChangeDate) ORDER BY UHId" %(stdate,enddate)
       ...
       if(len(get_roles_rec) > 0):
        #{
          output = '{"funtion":"get_temp_history","session_id":"%s","error_code":"0", "Response":"Successfully got %d history", \n "get_roles":' %(sid,len(get_roles_rec))
          output += '['
          counter = 0
          for rec in get_roles_rec:
          #{
