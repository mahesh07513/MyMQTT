...
def login(mosq,obj,msg):
    ...
    try:
       ...
       if results > 0:
          ...
          output = '{"function":"login","session_id":"%s","error_code":"0", "Response":"Successfully Authenticated for user: %s"}' %(sid,username)
	  return mqttc.publish('jts/oyo/error',output)
       else:
...
def add_unit(mosq,obj,msg):
    ...
    try:
       ...
       username    = data1['username']
       password    = data1['password']
       sqlq = "SELECT * FROM Oyo_Users WHERE Username='"+username+"' AND TransPass='"+password+"' AND Role=1"
       #sqlq = "SELECT * FROM Oyo_Users"
       #print "Checking Credentials : : ",sqlq
       cursor.execute(sqlq)
       results = cursor.fetchone()
          ...
          if add_rec1 > 0:
             ...
             if results3 > 0:
                ...
                if add_rec1 > 0 and add_rec2 > 0 :
                   ...
                   cursor.close()
                   db.close()
                   output = '{"function":"add_units","session_id":"%s","error_code":"0", "Response":"Successfully Registered Input Unit and Operate Unit"}' %(sid)
...
def get_roles(mosq,obj,msg):
    ...
    try:
       ...
       if(len(get_roles_rec) > 0):
        #{
          output = '{"funtion":"get_roles","session_id":"%s","error_code":"0", "Response":"Successfully got %d roles", \n "get_roles":' %(sid,len(get_roles_rec))
          output += '['
          counter = 0
...
def add_user(mosq,obj,msg):
    ...
    try:
       ...
       if((data1.get('Role') is None) or ((data1.get('Role') is not  None) and (len(data1['Role']) <= 0))):
           ...
           role = data1['Role']
       '''
       if((data1.get('TransPass') is None) or ((data1.get('TransPass') is not  None) and (len(data1['TransPass']) <= 0))):
           output_str += ", TransPass is mandatory"
           output = '{"function":"add_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
           return mqttc.publish('jts/oyo/error',output)
       else:
           tpass = data1['TransPass']
       '''
       ...
       if results > 0:
          ...
          #print "inserting ........."
          add_rec1=cursor.execute("""INSERT INTO Oyo_Users(Username,Password,TransPass,ChangeBy,Role,ChangeDate) VALUES (%s,%s,%s,%s,%s,%s)""",(uname,pass1,pass1,username,role,date))
          db.commit()
          if add_rec1>0:
...
def delete_user(mosq,obj,msg):
    ...
    try:
       ...
       return mqttc.publish('jts/oyo/error','{"function":"delete_user","error_code":"2","error_desc":"Response=invalid input, no proper JSON request"}')
    try:
       ...
       if((data1.get('username') is None) or ((data1.get('username') is not  None) and (len(data1['username']) <= 0))):
           output_str += ", username is mandatory"
           output = '{"function":"delete_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str )
           return mqttc.publish('jts/oyo/error',output)
       else:
           username = data1['username']
       ...
       if results > 0:
          ...
          #print 'Login Data not authorized so quiting ........Thanks'
          output_str += ",Your not Autherize to delete user"
          output = '{"function":"delete_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
          return mqttc.publish('jts/oyo/error',output)      
      
       if((data1.get('User_id') is None) or ((data1.get('User_id') is not  None) and (len(data1['User_id']) <= 0))):
           output_str += ", User_id is mandatory"
           output = '{"function":"delete_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str )
           return mqttc.publish('jts/oyo/error',output)
       else:
           uid = data1['User_id']
             
       sqlq12= "DELETE FROM Oyo_Users WHERE UserId='%s'" %(uid)
       print sqlq12
       errorcode=cursor.execute(sqlq12)
       db.commit()
       if errorcode > 0:
          output = '{"session_id":"%s","error_code":"0", "Response":"Successfully deleted the user"}' %(sid)
          return mqttc.publish('jts/oyo/error',output)
       else:
          output = '{"session_id":"%s","error_code":"-2", "error_desc": "Response= Unable to delete the user"}' %(sid)
          return mqttc.publish('jts/oyo/error',output)
        ...
        cursor.close()
        db.close()
        output = '{"session_id":"%s","error_code":"3", "error_desc": "Response=Failed to delete the user"}' %(sid)
        return mqttc.publish('jts/oyo/error',output)
    finally:
        cursor.close()
        db.close()
...
########################### get users  #####################################
def get_users(mosq,obj,msg):
    print "get_users......."
    db = MySQLdb.connect("localhost","oyoroom","ptljtsoyo","oyoDB")
    cursor = db.cursor()
    output_str = "get_users - Unable to Authenticate/get_users... "
    date1 = datetime.today()
    date = datetime.strftime(date1, '%Y-%m-%d %H:%M:%S.%f')
    try:
       data1 = json.loads(msg.payload)
       ...
       return mqttc.publish('jts/oyo/error','{"function":"get_users","error_code":"2","error_desc":"Response=invalid input, no proper JSON request"}')
    ...
    if results > 0:
       ...
       output = '{"function":"get_users","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
       return mqttc.publish('jts/oyo/error',output)
    try:
       sqlq2 = "SELECT UserId,Username,Role FROM Oyo_Users"
       ...
       cursor.execute(sqlq2)
       get_user_rec = cursor.fetchall()
       if(len(get_user_rec) > 0):
        #{
           output = '{"function":"get_users","session_id":"%s","error_code":"0", "Response":"Successfully got %d users", \n "get_users":' %(sid,len(get_user_rec))
           output += '['
           counter = 0
           for rec in get_user_rec:
           #{
              counter += 1
              if(counter == 1):
                output += '{"user_id":"%s","user_name":"%s","user_role":"%s"}' %(rec[0] ,rec[1],rec[2])
              else:
                output += ',\n {"user_id":"%s","user_name":"%s","user_role":"%s"}' %(rec[0] ,rec[1],rec[2])
           #}
           output += ']\n'
           output += '}'
        ...
        #}
           ...
           output = '{"function":"get_users","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to get the users, NO_DATA_FOUND"}' %(sid)
           return mqttc.publish('jts/oyo/error',output)

    except Exception, e:
        cursor.close()
        db.close()
        output = '{"function":"get_users","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to get the user"}' %(sid)
        return mqttc.publish('jts/oyo/error',output)
    finally:
        cursor.close()
        db.close()
############################# modify user ###################################
def modify_user(mosq,obj,msg):
    print "modify_user......."
    #print "this is add_users : ",str(msg.payload)
    db = MySQLdb.connect("localhost","oyoroom","ptljtsoyo","oyoDB")
    cursor = db.cursor()
    output_str = "modify_user - Unable to Authenticate/add_user... " 
    date1 = datetime.today()
    date = datetime.strftime(date1, '%Y-%m-%d %H:%M:%S.%f')
    try:
       data1 = json.loads(msg.payload)
       ...
       return mqttc.publish('jts/oyo/error','{"function":"modify_user","error_code":"2","error_desc":"Response=invalid input, no proper JSON request"}')

    if(data1.get('session_id') is None):
          output_str += ",session_id is mandatory"
          output = '{"function":"modify_user","error_code":"2", "error_desc": "Response=%s"}' %(output_str)
          return mqttc.publish('jts/oyo/error',output)
    ...
    if(data1.get('username') is None):
        output_str += ",username is mandatory"
        output = '{"function":"modify_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
        return mqttc.publish('jts/oyo/error',output)
    ...
    if results > 0:
       ...
       cursor.close()
       db.close()
       output_str += ",Your not Autherize to modify the user"
       output = '{"function":"modify_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
       return mqttc.publish('jts/oyo/error',output)
    try:
       ...
       if((data1.get('Mname') is None) or ((data1.get('Mname') is not  None) and (len(data1['Mname']) <= 0))):
           output_str += ", Mname is mandatory"
           output = '{"function":"modify_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
           return mqttc.publish('jts/oyo/error',output)
       else:
           mname = data1['Mname'] 

       if((data1.get('MPass') is None) or ((data1.get('MPass') is not  None) and (len(data1['MPass']) <= 0))):
           output_str += ", MPass is mandatory"
           output = '{"function":"modify_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
           return mqttc.publish('jts/oyo/error',output)
       else:
           mpass1 = data1['MPass'] 
 
       if((data1.get('MRole') is None) or ((data1.get('MRole') is not  None) and (len(data1['MRole']) <= 0))):
           output_str += ", MRole is mandatory"
           output = '{"function":"modify_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
           return mqttc.publish('jts/oyo/error',output)
       else:
           mrole = data1['MRole'] 
       ...
       upd_rec=cursor.execute ("""UPDATE Oyo_Users SET Username=%s,Password=%s,Role=%s,ChangeDate=%s,ChangeBy=%s WHERE UserId=%s""", (mname,mpass1,mrole,date,username,uid))
       db.commit()
       #sqlq1 = "SELECT * FROM Oyo_Users WHERE Username='"+uname+"' AND Password='"+pass1+"'"
       #sqlq = "SELECT * FROM Oyo_Users"
       #print "Checking input mac id ....... :",sqlq1
       ...
       #print "checking in opreate unit ..... :",results
       if upd_rec > 0:
          output = '{"function":"modify_user","session_id":"%s","error_code":"0", "Response":"Succesfully modify user"}' %(sid)
          cursor.close()
          db.close()
          return mqttc.publish('jts/oyo/error',output)
       else:
          output_str = "unable to modify user"
          output = '{"function":"modify_user","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str )
          return mqttc.publish('jts/oyo/error',output)    
    except MySQLdb.Error, e:
        try:
        ...
        cursor.close()
        db.close()
        output = '{"function":"modify_user","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to modify the User"}' %(sid)
        return mqttc.publish('jts/oyo/error',output)

############################# update Transaction Pass ########################
def update_trns_pass(mosq,obj,msg):
    print "update_trns_pass......."
    #print "this is add_users : ",str(msg.payload)
    db = MySQLdb.connect("localhost","oyoroom","ptljtsoyo","oyoDB")
    cursor = db.cursor()
    output_str = "update_trns_pass - Unable to Authenticate/update_trns_pass... " 
    date1 = datetime.today()
    date = datetime.strftime(date1, '%Y-%m-%d %H:%M:%S.%f')
    try:
       data1 = json.loads(msg.payload)
       ...
       return mqttc.publish('jts/oyo/error','{"function":"update_trns_pass","error_code":"2","error_desc":"Response=invalid input, no proper JSON request"}')

    if(data1.get('session_id') is None):
          output_str += ",session_id is mandatory"
          output = '{"function":"update_trns_pass","error_code":"2", "error_desc": "Response=%s"}' %(output_str)
          return mqttc.publish('jts/oyo/error',output)
    ...
    if(data1.get('username') is None):
        output_str += ",username is mandatory"
        output = '{"function":"update_trns_pass","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
        return mqttc.publish('jts/oyo/error',output)
    ...
    try:
       ...
       if((data1.get('TPass') is None) or ((data1.get('TPass') is not  None) and (len(data1['TPass']) <= 0))):
           output_str += ", TPass is mandatory"
           output = '{"function":"update_trns_pass","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str)
           return mqttc.publish('jts/oyo/error',output)
       else:
           tpass1 = data1['TPass'] 
       ...
       upd_rec=cursor.execute ("""UPDATE Oyo_Users SET TransPass=%s,ChangeDate=%s,ChangeBy=%s WHERE UserId=%s""", (tpass1,date,username,uid))
       db.commit()
       #sqlq1 = "SELECT * FROM Oyo_Users WHERE Username='"+uname+"' AND Password='"+pass1+"'"
       #sqlq = "SELECT * FROM Oyo_Users"
       #print "Checking input mac id ....... :",sqlq1
       ...
       #print "checking in opreate unit ..... :",results
       if upd_rec > 0:
          output = '{"function":"update_trns_pass","session_id":"%s","error_code":"0", "Response":"Succesfully modify user txn Pass"}' %(sid)
          cursor.close()
          db.close()
          return mqttc.publish('jts/oyo/error',output)
       else:
          output_str = "unable to modify user txn pass"
          output = '{"function":"update_trns_pass","session_id":"%s","error_code":"2", "error_desc": "Response=%s"}' %(sid,output_str )
          return mqttc.publish('jts/oyo/error',output)    
    except MySQLdb.Error, e:
        try:
        ...
        cursor.close()
        db.close()
        output = '{"function":"update_trns_pass","session_id":"%s","error_code":"3", "error_desc": "Response=Failed to modify the User txn pass"}' %(sid)
        return mqttc.publish('jts/oyo/error',output)
...
def test01(mosq,obj,msg):
    ...
    try:
       ...
       if(len(get_units_rec) > 0):
        #{
           output = '{"session_id":"%s","error_code":"0", "Response":"Successfully got %d units", \n "get_units_details":' %len(get_units_rec)
           output += '['
           counter = 0
           for rec in get_units_rec:
           #{
       ...
       '''
        ...
        cursor.close()
        db.close()
        output = '{"session_id":"%s","error_code":"3", "error_desc": "Response=Failed to get the units"}'
        return mqttc.publish('jts/oyo/error',output)
    finally:
        cursor.close()
        db.close()
...
def get_power(mosq,obj,msg):
       ...
       sid = data1['session_id']
       output = '{"function":"get_power","session_id":"%s","NP":"35","EP":"23","Diff":"12"}' %(sid)
################## publish response #################################################
...
mqttc.message_callback_add('jts/oyo/delete_user',delete_user)
mqttc.message_callback_add('jts/oyo/get_users',get_users)
mqttc.message_callback_add('jts/oyo/modify_user',modify_user)
mqttc.message_callback_add('jts/oyo/update_txn_pass',update_trns_pass)
